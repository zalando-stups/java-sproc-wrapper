...
import org.zalando.typemapper.namedresult.results.ClassWithPrimitivesAndMap;
import org.zalando.typemapper.namedresult.results.ClassWithSimpleTransformers;
    ...
    public static Collection<Object[]> generateData() throws SQLException {
        return Arrays.asList(
                new Object[][] {
                    ...
                    /* 3 */
                    {PgArray.ARRAY(1, 2, 3, 4).asJdbcArray("int4"), "{1,2,3,4}", Types.ARRAY},

                    /* 4 */
                    {PgArray.ARRAY(null, 2, 3, 4).asJdbcArray("int4"), "{NULL,2,3,4}", Types.ARRAY},

                    /* 5 */
                    {PgArray.ARRAY("a", "b").asJdbcArray("text"), "{a,b}", Types.ARRAY},

                    /* 6 */
                    {
                        PgArray.ARRAY("first element", "second \"quoted\" element").asJdbcArray("text"),
                        "{\"first element\",\"second \\\"quoted\\\" element\"}", Types.ARRAY
                    },

                    /* 7 */
                    {PgRow.ROW(1, 2).asPGobject("int_duplet"), "(1,2)", Types.OTHER},

                    /* 8 */
                    {
                        PgRow.ROW(1, 2, PgArray.ARRAY("a", "b")).asPGobject("int_duplet_with_text_array"), "(1,2,\"{a,b}\")",
                        Types.OTHER
                    },

                    /* 9 */
                    {
                        PgRow.ROW("a", "b", new int[] {1, 2, 3, 4}).asPGobject("text_duplet_with_int_array"),
                        "(a,b,\"{1,2,3,4}\")", Types.OTHER
                    },

                    /* 10 */
                    {
                        PgRow.ROW("a", null, PgArray.ARRAY(PgRow.ROW(1, 10), PgRow.ROW(2, 20), null)).asPGobject(
                    ...
                    /* 11 */
                    {
                        PgRow.ROW("a", null, PgArray.ARRAY(PgRow.ROW(1, 11), PgRow.ROW(2, 22), null)).asPGobject(
                    ...
                    /* 12 */
                    {
                        PgRow.ROW(1, new ClassWithPrimitives(1, 2L, 'c')).asPGobject("int_with_additional_type"),
                        "(1,\"(c,1,2)\")", Types.OTHER
                    },
